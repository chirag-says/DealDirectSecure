...
export default function Login() {
  ...
  const [forgotStep, setForgotStep] = useState(1); // 1: phone, 2: otp+password, 3: success
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (result.success && result.user) {
        ...
        const navQuery = pendingPropertyId 
          ? (redirectPath.includes("?") ? `&interestedPropertyId=${pendingPropertyId}` : `?interestedPropertyId=${pendingPropertyId}`) 
          : "";

        const finalUrl = redirectPath + navQuery;
  ...
  const handleSendOtp = async (e) => {
    e.preventDefault();
    const cleaned = (forgotPhone || "").trim();
    ...
    try {
      await api.post('/users/forgot-password', { phone: forgotPhone });
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-white/70">
                    {forgotStep === 1 && "Enter your phone number to receive OTP"}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        placeholder="10-digit mobile number"
                        value={forgotPhone}
                        onChange={(e) => setForgotPhone(e.target.value.replace(/[^0-9]/g, "").slice(0, 10))}
                        onInput={(e) => setForgotPhone((e.target.value || "").replace(/[^0-9]/g, "").slice(0, 10))}
                        autoComplete="tel-national"
                        inputMode="numeric"
                        required
                        className="w-full pl-10 pr-4 py-3 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-900 focus:border-blue-900 transition-colors outline-none text-slate-800"
                    ...
                    <p className="text-xs text-slate-500">
                      We&apos;ll send a 6-digit OTP via SMS to your registered phone number.
                    </p>
                  </div>
                ...
                <form onSubmit={handleResetPassword} className="space-y-5">
                  <div className="space-y-2">
                    ...
                    <input
                      ...
                      onInput={(e) => setForgotOtp((e.target.value || "").replace(/\D/g, '').slice(0, 6))}
                      maxLength={6}
                      autoComplete="one-time-code"
                      inputMode="numeric"
                      required
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-slate-500">OTP sent to +91 {forgotPhone}</span>
                      <button
                        type="button"
