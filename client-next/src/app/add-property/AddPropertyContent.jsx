...
export default function AddProperty() {
    ...
    useEffect(() => {
        ...
        if (!isAuthenticated || !authUser) {
            ...
            router.push(`/login?from=${encodeURIComponent(typeof window !== 'undefined' ? window.location.pathname + window.location.search : '')}`);
            return;
        }
        ...
        if (role === "agent") {
            ...
            // BUT skip redirect if we just submitted (reward modal may be showing)
            if (ownerHasProperty && !hasSubmittedRef.current) {
                toast.info(
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            const res = await api.post('/properties/add', submitData, {
            ...
            // Mark as submitted so the useEffect doesn't redirect while reward modal is open
    ...
    const renderStep1 = () => (
        <div className="space-y-5 sm:space-y-6">
            <div className="text-center">
                <h2 className="text-2xl sm:text-3xl font-bold text-gray-900">Basics</h2>
                <p className="text-gray-500 mt-1 sm:mt-2 text-sm sm:text-base px-2 hidden sm:block">Choose category and type. We'll show only relevant fields next.</p>
            </div>
            ...
            <main className="flex-1 p-3 pt-12 sm:p-6 sm:pt-6 md:p-12 max-w-5xl mx-auto w-full flex flex-col h-full">
                <div className="flex-1 pb-20 sm:pb-20 md:pb-0 relative">
                    <AnimatePresence mode="wait" custom={direction}>
                        <motion.div key={currentStep} custom={direction} variants={variants} initial="enter" animate="center" exit="exit" transition={{ x: { type: "spring", stiffness: 300, damping: 30 }, opacity: { duration: 0.2 } }} className="min-h-0 md:min-h-[520px]">
